...
#pragma GCC diagnostic pop
    ...
    constexpr size_t NUM_ELEMENTS_IN_MULTITHREAD_TEST = 1000;
    ...
    void *allocation_thread(void *arguments)
    {
        ...
        for (int i = 0; i < NUM_ELEMENTS_IN_MULTITHREAD_TEST; i++)
        {
            sizes[i] = rng() % 256;
        }

        for (int i = 50; i < NUM_ELEMENTS_IN_MULTITHREAD_TEST; i++)
        {
    ...
    {
        minstd::pmr::single_block_resource<true> upstream_resource(buffer, buffer_size);

        minstd::pmr::fixed_size_element_resource<32, 1024> resource(static_cast<minstd::pmr::memory_resource *>(&upstream_resource));
    ...
    TEST(FixedSizeElementMemoryResourceTests, GrowthUntilMemoryExhaustionTest)
    {
        ...
        minstd::pmr::fixed_size_element_resource<32, 2048> resource(static_cast<minstd::pmr::memory_resource *>(&upstream_resource));
        ...
        CHECK(buffer_size - resource.current_bytes_allocated() < (32 * 2048 * 3)); //  Remaining space should be less than three potential blocks
    }
    ...
    TEST(FixedSizeElementMemoryResourceTests, MultiThreadTest)
    {
        ...
        for (size_t i = 0; i < NUM_THREADS; i++)
        {
            for (int j = 0; j < NUM_ELEMENTS_IN_MULTITHREAD_TEST; j++)
            {
                ...
                if (!args[i].deleted_element[j])
                {
                    ...
                    total_number_of_bytes_allocated += args[i].sizes_allocated[j] % 32 == 0 ? args[i].sizes_allocated[j] : ((args[i].sizes_allocated[j] / 32) + 1) * 32;
                }
            }
        }
        ...
        printf("Percent Filled: %f\n", (double)total_number_of_bytes_allocated / (double)(resource.number_of_blocks() * 32 * 2048));
