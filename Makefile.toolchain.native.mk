...
# Use ?= so callers can override from the environment or command line.
...
# ── Include paths — auto-detected from the native compiler ────────────────────
...
# ── Test and coverage variables ───────────────────────────────────────────────

CPPUTEST_PATH ?= $(THIRD_PARTY)/cpputest

TEST_CFLAGS              := -g -DCPPUTEST_USE_STD_CPP_LIB=0 -DCPPUTEST_USE_STD_C_LIB=0 \
                             -DCHAR_BIT=8 -DCPPUTEST_HAVE_LONG_LONG_INT=0 \
                             -DCPPUTEST_USE_MEM_LEAK_DETECTION=0 -DINCLUDE_TEST_HELPERS
